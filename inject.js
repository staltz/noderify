...
function createDepStream(opts) {
  return moduleDeps({
    ...
    resolve: function(required, module, cb) {
      ...
      if (opts.replace[required] === false)
        throw new Error('should not resolve:' + required)
      return resolve(
        required,
        {
        ...
        function(err, file) {
          ...
          if (file) {
            if (!path.isAbsolute(file)) {
              ...
              if (c[0] !== '.')
                console.error('missing dependency. need to install:', c)
            } else if (file && file.substr(-5) === '.node') {
              opts.replace[required] = path.relative(
    ...
    for (var k in e.deps) {
      if (!e.deps[k]) delete e.deps[k]
      else e.deps[k] = path.relative(process.cwd(), e.deps[k])
    }
  })
}
...
module.exports = function(opts, cb) {
  ...
  deps.pipe(
    deterministic(function(err, content, deps, entry) {
      if (err) cb(err)
      else
        cb(
          null,
